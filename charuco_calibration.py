...
CHARUCOBOARD_ROWCOUNT = 6 #y축
CHARUCOBOARD_COLCOUNT = 9 #x 축
ARUCO_DICT = cv2.aruco.Dictionary_get(cv2.aruco.DICT_4X4_50)

# Create constants to be passed into OpenCV and Aruco methods
CHARUCO_BOARD = cv2.aruco.CharucoBoard_create(
    squaresX=CHARUCOBOARD_COLCOUNT, #x 방향의 체스판 사각형 수
    squaresY=CHARUCOBOARD_ROWCOUNT, #y 방향의 체스판 사각형 수
    squareLength=2.8, # 체스판 정사각형 한 변의 길이
    markerLength=2.2, #마커 측면 길이
...
images = glob.glob('./chruco_images/*.jpg')
...
numpy.savez("charuco_camera_calibration",ret=ret,mtx=mtx,dist=dist,rvecs=rvecs,tvecs=tvecs)
average_error=numpy.mean(perViewErrors)
print(average_error)